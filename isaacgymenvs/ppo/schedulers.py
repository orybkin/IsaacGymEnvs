...
class AdaptiveScheduler(RLScheduler):
    def __init__(self, kl_threshold = 0.008, coef=1.5):
        super().__init__()
        self.min_lr = 1e-6
        self.max_lr = 1e-2
    ...
    def update(self, current_lr, entropy_coef, epoch, frames, kl_dist, **kwargs):
        ...
        if kl_dist > (2.0 * self.kl_threshold):
            lr = max(current_lr / self.coef, self.min_lr)
        if kl_dist < (0.5 * self.kl_threshold):
            lr = min(current_lr * self.coef, self.max_lr)
...
class ExponentialScheduler(RLScheduler):
    def __init__(self, start_lr, gamma, **kwargs):
        super().__init__()
        self.gamma = gamma
        self.start_lr = start_lr

    def update(self, current_lr, entropy_coef, epoch, frames, kl_dist, **kwargs):
        return self.start_lr * (self.gamma ** epoch), entropy_coef     