...
from isaacgymenvs.redq_original.core import TanhGaussianPolicy, Mlp
...
class SACAgent(BaseAlgorithm):
    ...
    def __init__(self, base_name, params):
        ...
        act_dim = net_config['action_dim']
        hidden_sizes = [256, 256]
        policy = TanhGaussianPolicy(obs_dim, act_dim, hidden_sizes, action_limit=action_space.high[0].item()).to(self._device)

        q_net_list, q_target_net_list = [], []
        for q_i in range(2):
            new_q_net = Mlp(obs_dim + act_dim, 1, hidden_sizes).to(self._device)
            q_net_list.append(new_q_net)
            new_q_target_net = Mlp(obs_dim + act_dim, 1, hidden_sizes).to(self._device)
        ...
        class PolicyAdapter(nn.Module):
            ...
            def forward(self, obs):
                actions, _, _, log_prob, _, _ = self.policy.forward(obs)
                out = Dummy()
                out.rsample = lambda : actions
                out.sample = lambda : actions
                out.log_prob = lambda _: log_prob
                out.mean = actions
                return out
        ...
        class Adapter(nn.Module):
            def __init__(self, policy, q1, q2, q1_target, q2_target):
                super().__init__()
                self.sac_network = nn.Module()
                self.sac_network.actor = PolicyAdapter(policy)
                self.sac_network.critic = CriticAdapter(q1, q2)
                self.sac_network.critic_target = CriticAdapter(q1_target, q2_target)
                self.actor = self.sac_network.actor
                self.critic = self.sac_network.critic
                self.critic_target = self.sac_network.critic_target

        # self.model = Adapter(policy, q_net_list[0], q_net_list[1], q_target_net_list[0], q_target_net_list[1]).to(self._device)
        # self.model.sac_network.actor = PolicyAdapter(policy)
        # self.model.actor = self.model.sac_network.actor
                
        
        # self.model.sac_network.actor
        # policy 
        # import copy
        # actor = self.model.sac_network.actor
        # actor2 = copy.deepcopy(self.model.sac_network.actor)
        # policy(torch.ones(1,376).cuda(), deterministic=True)[0][0,0] # 0.2059
        # policy2 = copy.deepcopy(policy)
        # policy2.hidden_layers[0] = self.model.sac_network.actor.trunk[0]
        # policy2.hidden_layers[1] = self.model.sac_network.actor.trunk[2]
        # policy2.last_fc_layer.weight = nn.Parameter(self.model.sac_network.actor.trunk[4].weight[:17])
        # policy2.last_fc_layer.bias = nn.Parameter(self.model.sac_network.actor.trunk[4].bias[:17])
        # policy2.last_fc_log_std.weight = nn.Parameter(self.model.sac_network.actor.trunk[4].weight[17:])
        # policy2.last_fc_log_std.bias = nn.Parameter(self.model.sac_network.actor.trunk[4].bias[17:])

        # actor(torch.ones(1,376).cuda()).loc[0,0] # -1.0019
        # actor.trunk(torch.ones(1,376).cuda())[0,0] # -1.0019
        # policy2(torch.ones(1,376).cuda(), deterministic=True)[0][0,0] # 0.0489

        # # core
        # x = torch.ones(1,376).cuda()
        # x = policy2.hidden_activation(policy2.hidden_layers[0](x))
        # x = policy2.hidden_activation(policy2.hidden_layers[1](x))
        # mean = policy2.last_fc_layer(x) # 0.1230
        # torch.tanh(mean) * policy2.action_limit # 0.0489
